...
version 3.2: 01.01.18 added user photos
version for ZBang 4.0: 03.01.18 added account SIDs
version for ZBang 4.1: 12.6.18 few modifications
version for ZBang 4.2: 02.09.18 update with things from ACLight2 on GitHub (like nested group)
...
function Start-domainACLsAnalysis {
    ...
    ###############################################################################################################################################
    if (-not $multiLayered) {
        # The root of the domain and AdminSDHolder
...
function Update-PermissionsByAccounts {
    ...
    Import-Csv $inputCSV | Where-Object {$_} | ForEach-Object {
        ...
        foreach($account in $privilegedAccountList) {
            ...
            if ($ObjectSID) {
	            $ObjectAccount = Convert-SidToName $ObjectSID
            }

			$userphoto = ""
			try
			{
				if (-not $dbUserPhotos.ContainsKey($account)) {    
					 ## get account photo in base64
					$userphoto = ""
					$samaccountname = $account
					$bytes = GetMemberThumbnail([string]$samaccountname)
					if ($bytes -ne $null) {
						$userphoto = [Convert]::ToBase64String($bytes)
					}
					$dbUserPhotos.add($account,1)
				}
			}
			catch { }
            if (($_.EntityName -eq $account) -or ($_.EntityGroupMembers -eq $account)){
                $accountPermissionLine = [PSCustomObject][ordered] @{
                    ...
                    ObjectAccount           = [string]$ObjectAccount
                    ObjectDN                = [string]$_.ObjectDN
                    ObjectOwner             = [string]$_.ObjectOwner
            ...
            elseif (($account -like "*-notDomainAccount") -or ($account -like "*-group")) {
				$sidAccount = Convert-NameToSid $account
                $layerNumber = $layersDB[$sidOwner]
                $accountPermissionLine = [PSCustomObject][ordered] @{
                    Layer                   = [string]$layerNumber
                    Domain                  = [string]$Domain
...
"
        ...
        Write-Output "Great, the scan was started - version 4.2.`nIt could take a while, (5-30+ mins) depends on the size of the network`n"
